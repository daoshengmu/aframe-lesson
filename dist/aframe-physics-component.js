...
/***/ function(module, exports, __webpack_require__) {
	...
	AFRAME.registerComponent('physics-rigid-vehicle', {
	  dependendencies: ['physics-body'],

	  schema: {
	    chassisShape: { type: 'vec3' },
	    mass: { default: 1 },
	    wheelRadius: { default: 3 }
	  },
	  ...
	  init: function () {
	    ...
	    var chassisShape = new CANNON.Box(new CANNON.Vec3(5, 2, 5));
	    var chassisBody = new CANNON.Body({ mass: data.mass });
	    var position = el.object3D.position;    
	    var vehicle = this.vehicle = new CANNON.RigidVehicle({ chassisBody: chassisBody });
	    chassisBody.addShape(chassisShape, centerOfMassAdjust);
	    chassisBody.position.copy(position);

	    var axisWidth = 7;
	    var wheelShape = new CANNON.Sphere(data.wheelRadius);
	    var down = new CANNON.Vec3(0, 0, -1);
	    var wheelMaterial = new CANNON.Material("wheelMaterial");

	    var wheelBody = new CANNON.Body({ mass: data.mass, material: wheelMaterial });
	    wheelBody.addShape(wheelShape);
	    vehicle.addWheel({
	        body: wheelBody,
	        position: new CANNON.Vec3(5, -1, axisWidth/2).vadd(centerOfMassAdjust),
	   ...
	   // Wait for scene to load to initialize physics world.
	    sceneEl.addEventListener('loaded', function () {
	      ...
	      var world = self.world = sceneEl.components['physics-world'].world;
	      self.body = self.el.components['physics-body'];
	      self.control = self.el.components['wasd-physics-controls'];
	  ...
	  update: function (oldData) {
	    ...
	    // Need to update physics body velocity
	    console.log('RigidVehicle update...');

	    if (this.body && this.control) {
	      var velocity = this.body.body.velocity;
	      // if (!this.world) { return; }
	      // var diffData = diff(this.data, oldData || this.data);
